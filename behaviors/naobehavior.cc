...
bool NaoBehavior::isOurBall(){
    VecPosition temp;
    float oppoDistanceToBal;
    float oppoClosestToBall=50.0;
    for(int i=12;i<23;++i){
        temp= worldModel ->getOpponent(i);
        temp.setZ(0);
        oppoDistanceToBal=temp.getDistanceTo(ball);
        if(oppoDistanceToBal<oppoClosestToBall){			
            oppoClosestToBall=	oppoDistanceToBal;
        }
    }
    VecPosition me = worldModel->getWorldObject(findPlayerDistance2Ball().first)->pos;
    float ourClosestToBall=me.getDistanceTo(ball);
    if(abs(me.getAngleBetweenPoints(ball,me))<45){
        ourClosestToBall+=0.5;
    }
    else if (abs(me.getAngleBetweenPoints(ball,me))<90){
        ourClosestToBall+=0.5;
    }
    else {
        ourClosestToBall+=1;
    }
    if (ourClosestToBall<oppoClosestToBall){
        return true;
        cout<<"我方可以控球"<<endl;
    }
    else{
...
bool NaoBehavior::isOppoNearby(int near)
{
    VecPosition nearestTeammate = worldModel -> getTeammate(near);
    for(int i=12;i<23;++i)
    {
        VecPosition oppo= worldModel ->getOpponent(i);
        if(ball.getDistanceTo(oppo)<=1.5 && oppo.getX() - nearestTeammate.getX() < 0)
        {			
            return true;	
        }
    }
...
SkillType NaoBehavior::onBallSkill(VecPosition position)
{
    VecPosition target = VecPosition(HALF_FIELD_X,0,0);

    if (me.getDistanceTo(ball) > 1) {
        // Far away from the ball so walk toward target offset from the ball
        //VecPosition approachBallTar = ball - kickDirection*atof(namedParams.find("drib_target")->second.c_str());
        return goToTarget(ball);
    }
   else {
        // Here is how we dribble
        int currentKickType = KICK_DRIBBLE;
        ...
        kickDirection = (position - ball).normalize();
        ...
        if (angleError < angleThreshold) {
            //worldModel->getRVSender()->drawLine(kickDirection.getX(), kickDirection.getY(), me2Ball.getX(), me2Ball.getY(),255,0,0);
        ...
        //target = ballTarget - (VecPosition(kickDirection)  * 0.18);
        target.setZ(0);
        VecPosition originalTarget = target;
        target = navigateAroundBall(target, .5 /*PROXIMITY_TRESH*/,atof(namedParams.find("drib_coll_thresh")->second.c_str()));
        //target = navigateAroundBall(target, .5 /*PROXIMITY_TRESH*/,0.34);
        ...
        cout<<"调整:"<<abs(me.getAngleBetweenPoints(kickDirection, me2Ball))<<endl;
        return goToTargetRelative(localTarget, turnAngle, 1.0, false /*fAllowOver180Turn*/, WalkRequestBlock::PARAMS_POSITIONING);

   }     
}

bool NaoBehavior::isOpponentSpeedZero(){
    VecPosition temp2;
    double oppoClosestToBall=50;
    WorldObject *oppovision=worldModel->getWorldObject(WO_OPPONENT1);
    for(int i=12;i<23;++i)
    {
        double oppoDistanceToBall;
        WorldObject *temp= worldModel->getWorldObject(i);
        temp2=worldModel->getOpponent(i);
        temp2.setZ(0);
        oppoDistanceToBall=temp2.getDistanceTo(ball);
        if(oppoDistanceToBall<oppoClosestToBall){			
            oppovision=temp;
        }
    }
    SIM::Point2D oppoVel = oppovision->absVel;
    SIM::Point2D ballVel = worldModel->getWorldObject(WO_BALL)->absVel;
    if(abs(ballVel.getX())<0.05 || abs(ballVel.getY())<0.1){
        cout<<"球静止"<<endl;
        if(abs(oppoVel.getX())<0.1 && abs(oppoVel.getY()<0.1)){
            cout<<"对方onball速度小于0.1"<<endl;
            return true;
        }
        else{
            cout<<"oppoVel.getX():  "<<oppoVel.getX()<<endl;
            cout<<"oppoVel.getY():  "<<oppoVel.getY()<<endl;
            return false;
        }
    }
    else{
